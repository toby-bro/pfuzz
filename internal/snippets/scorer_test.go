...
func TestWeightedSelection(t *testing.T) {
	...
	for name, count := range counts {
		if name == "assert_property_mod" || name == "assume_property_mod" || name == "cover_property_mod" {
			fmt.Printf("  %s: %d (%.1f%%)\n", name, count, float64(count)*100.0/float64(totalIterations))
		}
	}
	...
	if assertCount < assumeCount {
		t.Logf("Warning: assert_property_mod (score 0.67) was selected %d times vs assume_property_mod (score 0.33) %d times", assertCount, assumeCount)
		// Only fail if the difference is very significant (more than 3:1 ratio)
		if assumeCount > assertCount*3 {
			t.Errorf("assert_property_mod should generally be selected more than assume_property_mod due to higher score")
		}
	}
	...
	if totalSnippetsWithScores == 0 {
		t.Errorf("None of our test snippets with scores were selected, which suggests scoring isn't working")
	}
	
	t.Logf("Successfully tested weighted selection with %d total selections of scored snippets", totalSnippetsWithScores)
}