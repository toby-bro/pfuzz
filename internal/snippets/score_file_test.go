...
func TestScoreFileReadWrite(t *testing.T) {
	...
	if loadedScore.NumSimulators != testScore.NumSimulators {
		t.Errorf(
			"NumSimulators mismatch: got %d, want %d",
			loadedScore.NumSimulators,
			testScore.NumSimulators,
		)
	}
	if loadedScore.NumSynthesizers != testScore.NumSynthesizers {
		t.Errorf(
			"NumSynthesizers mismatch: got %d, want %d",
			loadedScore.NumSynthesizers,
			testScore.NumSynthesizers,
		)
	}
	if loadedScore.SimulatorScore != testScore.SimulatorScore {
		t.Errorf(
			"SimulatorScore mismatch: got %d, want %d",
			loadedScore.SimulatorScore,
			testScore.SimulatorScore,
		)
	}
	if loadedScore.SynthesizerScore != testScore.SynthesizerScore {
		t.Errorf(
			"SynthesizerScore mismatch: got %d, want %d",
			loadedScore.SynthesizerScore,
			testScore.SynthesizerScore,
		)
	}
	if loadedScore.MaximalScore != testScore.MaximalScore {
		t.Errorf(
			"MaximalScore mismatch: got %d, want %d",
			loadedScore.MaximalScore,
			testScore.MaximalScore,
		)
	}
	if loadedScore.ReachedScore != testScore.ReachedScore {
		t.Errorf(
			"ReachedScore mismatch: got %d, want %d",
			loadedScore.ReachedScore,
			testScore.ReachedScore,
		)
	}
	...
	if loadedScore.Probability != expectedProbability {
		t.Errorf(
			"Probability mismatch: got %f, want %f",
			loadedScore.Probability,
			expectedProbability,
		)
	}
}
